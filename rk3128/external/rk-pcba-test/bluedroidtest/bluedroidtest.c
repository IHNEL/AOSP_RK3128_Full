...
int HAL_load(void)
{
    ...
    err = hw_get_module(BT_HARDWARE_RTK_MODULE_ID
		    /* BT_HARDWARE_MODULE_ID */, (hw_module_t const**)&module);
    if (err == 0)
    {
        err = module->methods->open(module, BT_HARDWARE_RTK_MODULE_ID
			/* BT_HARDWARE_MODULE_ID */, &device);
        if (err == 0) {
    ...
    bdt_log("HAL library loaded %s (%s)", BT_HARDWARE_RTK_MODULE_ID, strerror(err));
