...
int cpu_set_mode(char *mode)
{
	...
	if (cpu_num0 > 0) {
		...
		sprintf(command, "echo " "%s" " > %s", mode,
			_CPU_0_FREQ_GOVERNOR);
		__system(command);
	}
...
void *cpufreq_test(void *argv)
{
	...
	while (1) {
		...
		case 10:	/*if end time */
			if (cpu_num0 > 0)
				set_curl_freq(&cpu_0, cpu_num0,
					      _CPU_0_FREQ_SET);
			if (cpu_num1 > 0)
				set_curl_freq(&cpu_1, cpu_num1,
					      _CPU_1_FREQ_SET);
			if (cpu_num2 > 0)
				set_curl_freq(&cpu_2, cpu_num2,
					      _CPU_2_FREQ_SET);
			if (cpu_num3 > 0)
				set_curl_freq(&cpu_3, cpu_num3,
					      _CPU_3_FREQ_SET);
			break;
...
void *add_use_cpu(void)
{
	...
	while (1) {
		...
		anglePitch =
		    atan2(angleData[0],
			  sqrt((int)(angleData[1] * angleData[1]) +
			       (int)(angleData[2] * angleData[2])));
		angleRoll =
		    atan2(angleData[1],
			  sqrt((int)(angleData[0] * angleData[0]) +
			       (int)(angleData[2] * angleData[2])));

		accPitch =
		    atan2(accData[0],
			  sqrt((int)(accData[1] * accData[1]) +
			       (int)(accData[2] * accData[2])));
		accRoll =
		    atan2(accData[1],
			  sqrt((int)(accData[0] * accData[0]) +
			       (int)(accData[2] * accData[2])));
		...
		if (anglePitch > accPitch)
			...
			curr_angle[0] =
			    (int)((anglePitch - accPitch) + 360) % 360;

		if (angleRoll > accRoll)
			curr_angle[1] = (int)(angleRoll - accRoll);
		else
			curr_angle[1] =
			    (int)((angleRoll - accRoll) + 360) % 360;
	}
...
int get_boot_mode(void)
{
	...
	memset(command, 0, sizeof(command));
	sprintf(command,
		"busybox dmesg -s 50000 | busybox grep \"Boot mode:\" > data/boot_mode.txt");
...
static const char *boot_mode_name(int mode)
{
	...
	case BOOT_MODE_NORMAL:
		return "NORMAL";
	case BOOT_MODE_FACTORY2:
		return "FACTORY2";
	case BOOT_MODE_RECOVERY:
		return "RECOVERY";
	case BOOT_MODE_CHARGE:
		return "CHARGE";
	case BOOT_MODE_POWER_TEST:
		return "POWER_TEST";
	case BOOT_MODE_OFFMODE_CHARGING:
		return "OFFMODE_CHARGING";
...
void *cpu_test(void *argv)
{
	...
	if (bootmode == BOOT_MODE_PANIC || bootmode == BOOT_MODE_WATCHDOG) {
		...
		tc_info->result = -1;
		ui_print_xy_rgba(0, tc_info->y, 0, 0, 255, 255,
				 "CPU:[%s] { %s }\n", PCBA_FAILED,
				 boot_mode_name(bootmode));
	} else {
		tc_info->result = 1;
	...
	while (1) {
		...
		if (cpu_space > 10) {
			printf("i:%d,space:%d\r\n", i, cpu_space);
			pthread_create(&pid_stress[i], NULL, add_use_cpu,
				       (void *)i);
			i++;
		}
